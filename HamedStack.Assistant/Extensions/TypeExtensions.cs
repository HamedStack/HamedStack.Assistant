...
public static class TypeExtensions
{
    ...
    public static LambdaExpression CreateExpression(this Type type, string propertyName)
    {
        ...
        foreach (var member in propertyName.Split('.'))
        {
            if (member.EndsWith("]"))
            {
                var indexStart = member.IndexOf("[", StringComparison.Ordinal);
                var indexEnd = member.IndexOf("]", StringComparison.Ordinal);
                var arrayName = member[..indexStart];
                var indexValue = int.Parse(member.Substring(indexStart + 1, indexEnd - indexStart - 1));

                body = Expression.PropertyOrField(body, arrayName);

                var toListMethod = typeof(Enumerable).GetMethod("ToList")?.MakeGenericMethod(body.Type.GetGenericArguments()[0]);
                if (toListMethod != null) body = Expression.Call(toListMethod, body);

                var itemProperty = typeof(List<>).MakeGenericType(body.Type.GetGenericArguments()[0]).GetProperty("Item");
                body = Expression.MakeIndex(body, itemProperty, new[] { Expression.Constant(indexValue) });
            }
            else
            {
    ...
    public static IDictionary<string, int> EnumToDictionary(this Type @this)
    {
        ...
        var values = Enum.GetValues(@this).Cast<int>().ToArray();

        return (from i in Enumerable.Range(0, names.Length)
                select new { Key = names[i], Value = values[i] })
    ...
    public static Type? FindInterfaceThatCloses(this Type type, Type openType)
    {
        ...
        var typeInfo = type.GetTypeInfo();
        if (typeInfo is { IsInterface: true, IsGenericType: true } && type.GetGenericTypeDefinition() == openType)
        {
            return type;
        }
    ...
    public static string GetCleanName(this Type typeRef, GenericPresentationMode genericMode = GenericPresentationMode.Normal, bool fullName = false)
    {
        ...
        static string GetCleanedName(Type typeRef, bool fullName = false)
        {
            ...
            if (!fullName && typeRef.Namespace != null && cleanedName.StartsWith(typeRef.Namespace))
                cleanedName = cleanedName[(typeRef.Namespace.Length + 1)..];

            if (!typeRef.IsGenericType)
                return cleanedName;
            return cleanedName[..cleanedName.LastIndexOf('`')]
    ...
    public static bool IsConcrete(this Type? type)
    {
        ...
        var typeInfo = type.GetTypeInfo();
        return typeInfo is { IsAbstract: false, IsInterface: false };
    }
    ...
    public static string PrettyPrint(this Type type, Func<Type, string> selector)
    {
        ...
        if (tickLocation >= 0)
        {
            typeName = typeName[..tickLocation];
        }
    ...
    private static string GetPrettyNameOf(this Type? type, List<Type?> traversed)
    {
        traversed.Add(type);
        ...
        {
            if (!traversed.Contains(type.DeclaringType))
                prefixName = GetPrettyNameOf(type.DeclaringType, traversed) + ".";
        }
        ...
        {
            var genericArgNames = type.GetGenericArguments().Select(type1 => GetPrettyNameOf(type1, new List<Type?>()));
            var idx = type.Name.IndexOf('`');
            var typename = idx > 0 ? type.Name[..idx] : type.Name;
            return $"{prefixName}{typename}<{string.Join(", ", genericArgNames.ToArray())}>";
        }
