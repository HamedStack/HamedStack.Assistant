...
public static class StringExtensions
{
    ...
    public static IEnumerable<string> Chunk(this string str, int maxLength, bool breakWord)
    {
        ...
        else
        {
            ...
            while (startIndex < str.Length)
            {
                ...
                var chunk = str.Substring(startIndex, Math.Min(length, str.Length - startIndex));
                yield return chunk;
    ...
    /// <summary>
    /// Splits the string into chunks of the given maximum length, with overlapping.
    /// </summary>
    ...
    public static IEnumerable<string> ChunkBy(this string str, int maxLength, int overlap)
    {
        if (string.IsNullOrEmpty(str)) throw new ArgumentException("The string cannot be null or empty.");
        if (maxLength <= 0) throw new ArgumentException("Max length should be greater than zero.");
        if (overlap < 0 || overlap >= maxLength) throw new ArgumentException("Overlap should be non-negative and less than maxLength.");

        for (var i = 0; i < str.Length; i += maxLength - overlap)
        {
            if (i + maxLength <= str.Length)
            {
                yield return str.Substring(i, maxLength);
    ...
    /// <summary>
    /// Splits the string into chunks of the given maximum length.
    /// </summary>
    /// <param name="str">The source string to be chunked.</param>
    /// <param name="maxLength">The maximum length for each chunk.</param>
    /// <exception cref="ArgumentException">Thrown when the input string is null or empty.</exception>
    ...
    public static string GetAfter(this string @this, string value)
    {
        ...
        return @this[(@this.IndexOf(value, StringComparison.Ordinal) + value.Length)..];
    }

    public static string GetBefore(this string @this, string value)
    {
        if (@this.IndexOf(value, StringComparison.Ordinal) == -1) return "";
        return @this[..@this.IndexOf(value, StringComparison.Ordinal)];
    }
    ...
    public static bool IsDirectoryPath(this string input)
    {
        if (string.IsNullOrWhiteSpace(input))
            return false;

        var windowsDirPattern = @"^[a-zA-Z]:[\\/](?:[^\.\/\\\*?\""<>\|]+[\\/]?)*$";
        var unixDirPattern = @"^\/(?:[^\.\/\*?\""<>\|]+\/?)*$";

        return Regex.IsMatch(input, windowsDirPattern)
               || Regex.IsMatch(input, unixDirPattern);
    }
    ...
    public static bool IsFilePath(string input)
    {
        ...
        return Regex.IsMatch(input, windowsPathPattern)
               || Regex.IsMatch(input, unixPathPattern);
    }
    ...
    public static bool IsUri(this string input)
    {
        if (Uri.TryCreate(input, UriKind.Absolute, out var result))
        {
            return result.Scheme == Uri.UriSchemeHttp
                   ...
                   || result.Scheme == Uri.UriSchemeNetTcp
                   || result.Scheme == Uri.UriSchemeNetPipe;
        }
        return false;
    }
    ...
    {
        return @this[(removeChar ? @this.IndexOf(c) + 1 : @this.IndexOf(c))..];
    }

    public static string RemoveBeforeLastIndex(this string @this, char c, bool removeChar = true)
    {
        return @this[(removeChar ? @this.LastIndexOf(c) + 1 : @this.LastIndexOf(c))..];
    }
