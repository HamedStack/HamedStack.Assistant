...
public static partial class ObjectExtensions
{
        ...
        where TResult : notnull
    {
        ...
        if (obj1 is not T obj2) return false;

        var result1 = selector(obj);
        var result2 = selector(obj2);
    ...
    public static Expression ToConstantExpression<T>(this T value)
    {
        return Expression.Constant(value, typeof(T));
    }

    public static Expression<Func<T>> ToExpression<T>(this T value)
    {
        var constant = Expression.Constant(value, typeof(T));
        return Expression.Lambda<Func<T>>(constant);
    }
    ...
    public static LambdaExpression ToLambdaExpression<T>(this T value)
    {
        var constant = Expression.Constant(value, typeof(T));
        return Expression.Lambda<Func<T>>(constant);
    }
    ...
    public static Expression<Func<object>> ToObjectExpression(this object value)
    {
    ...
    public static LambdaExpression ToObjectLambdaExpression(this object value)
    {
        var constant = Expression.Constant(value, typeof(object));
        return Expression.Lambda<Func<object>>(constant);
    }
