...
public static class ExpressionExtensions
{
    ...
    public static List<Expression> Flatten(this Expression expression)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            {
                if (child != null) stack.Push((Expression)child);
            }
        }

        return result;

        static IEnumerable<object?> GetChildren(Expression expression)
        {
            foreach (var field in expression.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            {
                ...
                {
                    yield return field.GetValue(expression);
                }
                else if (field.FieldType.IsAssignableFrom(typeof(IEnumerable<Expression>)))
                {
                    if (field.GetValue(expression) is IEnumerable<Expression> children)
                    {
    ...
    public static Expression<Func<T, bool>> Invoke<T>(this Expression<Func<T, bool>> expr, T value)
    {
        ...
        var body = visitor.Visit(expr.Body) ?? throw new ArgumentNullException($"visitor.Visit({nameof(expr)}.Body)");
    ...
    public class ExpressionContainsVisitor : ExpressionVisitor
    {
        ...
        public bool Consists { get; private set; }

        public override Expression? Visit(Expression? node)
        {
            if (!Consists && node != null && node.Equals(_subexpression))
            {
                Consists = true;
            }
    ...
    private class ExpressionTransformVisitor : ExpressionVisitor
    {
        ...
        public override Expression? Visit(Expression? node)
        {
            return base.Visit(_transformFunc?.Invoke(node));
        }
    }
